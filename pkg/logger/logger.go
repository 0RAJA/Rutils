...
func NewLogger(level string) *Log {
	...
	lowFileWriteSyncer := getLogWriter(initLog.LogSavePath + initLog.LowLevelFile)
	highFileWriteSyncer := getLogWriter(initLog.LogSavePath + initLog.HighLevelFile)
	encoder := getEncoder()
